...
local ViewGroup=反射.取类("android.view.ViewGroup")
local String=反射.取类("java.lang.String")
local Gravity=反射.取类("android.view.Gravity")
local OnClickListener=反射.取类("android.view.View$OnClickListener")
local TypedValue=反射.取类("android.util.TypedValue")
local BitmapDrawable=反射.取类("android.graphics.drawable.BitmapDrawable")
local ArrayListAdapter=反射.取类("android.widget.ArrayListAdapter")
local ArrayPageAdapter=反射.取类("android.widget.ArrayPageAdapter")
local ScaleType=反射.取类("android.widget.ImageView$ScaleType")
local TruncateAt=反射.取类("android.text.TextUtils$TruncateAt")
local scaleTypes=ScaleType.values()
local android_R=反射.取类("android.R")
android={R=android_R}

local Context=反射.取类 "android.content.Context"
local DisplayMetrics=反射.取类 "android.util.DisplayMetrics"
...
local ver = 反射.取类("android.os.Build").VERSION.SDK_INT;
    ...
    if type(v)=="string" then
      ...
      elseif rawget(root,v) or rawget(_G,v) then
        ...
        if v:find("%.9%.png") then
          setBackground(view,绘画.点九图转绘画(图片.读取(v)))
        else
          setBackground(view,BitMapDrawable(图片.读取(v)))
        end
      end
    elseif type(v)=="userdata" then
      setBackground(view,v)
...
local function 创建布局(t,root,group)
  if type(t)=="string" then
    t=require(t)
  elseif type(t)~="table" then
    error(string.format("创建布局 error: Fist value Must be a table, checked import layout.",0))
    end
  root=root or _G
  local view,style
  ...
  if not t[1] then
    error(string.format("创建布局 error: Fist value Must be a Class, checked import package.\n\tat %s",dump2(t)),0)
  end

  if style then
    view = t[1](当前界面,nil,style)
  else
    view = t[1](当前界面) --创建view
  end
