...
public class Selector {
     ...
     */
    public static 元素组 select(String query, 元素 root) {
     ...
     * @return matching elements, empty if none
     */
    public static 元素组 select(Evaluator evaluator, 元素 root) {
     ...
     * @return matching elements, empty if none
     */
    public static 元素组 select(String query, Iterable<元素> roots) {
        Validate.notEmpty(query);
        Validate.notNull(roots);
        Evaluator evaluator = QueryParser.parse(query);
        ArrayList<元素> elements = new ArrayList<>();
        IdentityHashMap<元素, Boolean> seenElements = new IdentityHashMap<>();
        // dedupe elements by identity, not equality

        for (元素 root : roots) {
            final 元素组 found = select(evaluator, root);
            for (元素 el : found) {
        ...
        return new 元素组(elements);
    }

    // exclude set. package open so that Elements can implement .not() selector.
    static 元素组 filterOut(Collection<元素> elements, Collection<元素> outs) {
        元素组 output = new 元素组();
        for (元素 el : elements) {
            boolean found = false;
            for (元素 out : outs) {
     ...
     */
    public static 元素 selectFirst(String cssQuery, 元素 root) {
