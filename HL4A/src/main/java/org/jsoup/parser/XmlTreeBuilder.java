...
public class XmlTreeBuilder extends TreeBuilder {
    ...
    protected void initialiseParse(Reader input, String baseUri, ParseErrorList errors, ParseSettings settings) {
        ...
        doc.outputSettings().syntax(文档.OutputSettings.Syntax.xml);
    }
    ...
    元素 insert(Token.StartTag startTag) {
        ...
        元素 el = new 元素(tag, baseUri, settings.normalizeAttributes(startTag.attributes));
        insertNode(el);
    ...
    void insert(Token.Comment commentToken) {
        ...
        if (commentToken.bogus) { // xml declarations are emitted as bogus comments (which is right for html, but not xml)
            ...
            if (data.length() > 1 && (data.startsWith("!") || data.startsWith("?"))) {
                文档 doc = Jsoup.parse("<" + data.substring(1, data.length() -1) + ">", baseUri, Parser.xmlParser());
                元素 el = doc.child(0);
